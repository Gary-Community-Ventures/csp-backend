...
@bp.post("/provider/refresh-payment-status")
@auth_required(ClerkUserType.PROVIDER)
def refresh_my_payment_status():
    """Manually trigger a refresh of the provider's payment status from Chek."""
    user = get_provider_user()
    provider_id = user.user_data.provider_id
    ...
    if not provider_payment_settings:
        return jsonify({"error": "Provider payment settings not found. Please complete onboarding first."}), 404
    ...
    try:
        ...
        return (
            jsonify(
                {"message": "Payment status refresh has been queued", "job_id": job.id, "provider_id": provider_id}
            ),
            202,
        )

    except Exception as e:
        current_app.logger.error(f"Failed to enqueue refresh for provider {provider_id}: {e}")
        return jsonify({"error": "Failed to queue refresh job"}), 500


@bp.get("/test/chek-programs")
def test_list_chek_programs():
    """Test endpoint to list all Chek programs for debugging - NO AUTH."""
    try:
        from app.integrations.chek.schemas import ProgramListResponse

        # Get the Chek service
        payment_service = current_app.payment_service
        chek_client = payment_service.chek_service.client

        # Call the list_programs API
        response_data = chek_client.list_programs()

        # Parse and validate the response
        programs = ProgramListResponse.model_validate(response_data)
                ...
                "programs": [
                    {
                        ...
                        "balance_dollars": f"${program.balance / 100:.2f}",
                        "created_at": program.created_at.isoformat(),
                        "parent": program.parent.display_string,
                    }
        ...
        current_app.logger.error(f"Failed to list Chek programs: {e}")
        return jsonify({"success": False, "error": str(e), "message": "Failed to retrieve programs from Chek"}), 500
