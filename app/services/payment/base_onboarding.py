...
class BaseOnboarding(ABC):
    ...
    @abstractmethod
    def get_existing_settings(self, external_id: str) -> Optional[Union[ProviderPaymentSettings, FamilyPaymentSettings]]:
    ...
    def update_settings_from_status(
        self, 
        settings: Union[ProviderPaymentSettings, FamilyPaymentSettings],
        status: Dict
    ) -> None:
    ...
    def onboard(self, external_id: str) -> Union[ProviderPaymentSettings, FamilyPaymentSettings]:
        ...
        try:
            ...
            if existing_chek_user:
                ...
                current_app.logger.info(
                    f"Created Chek user {chek_user_response.id} for {entity_type} {external_id}"
                )
            ...
            current_app.logger.info(
                f"Successfully onboarded {entity_type} {external_id} with Chek user {chek_user_id}"
            )
