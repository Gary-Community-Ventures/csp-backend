...
class AllocatedCareDay(db.Model, TimestampMixin):
    ...
    type = db.Column(db.Enum('Full Day', 'Half Day', name='care_day_type'), nullable=False)
    
    # Calculated fields
    amount_dollars = db.Column(db.Numeric(10, 2), nullable=False)
    day_count = db.Column(db.Numeric(4, 1), nullable=False)  # 1.0 for full day, 0.5 for half day
    ...
    __table_args__ = (
        ...
        db.UniqueConstraint('care_month_allocation_id', 'date', name='unique_allocation_date'),
    )
    ...
    @staticmethod
    def create_care_day(allocation: 'MonthAllocation', provider_id: int, care_date: date, day_type: str):
        """Create a new care day with proper validation"""
        # Check if allocation can handle this care day
        if not allocation.can_add_care_day(day_type):
        ...
        if existing and existing.is_deleted:
            ...
            existing.amount_dollars = get_care_day_cost(day_type)
            existing.day_count = 1.0 if day_type == "Full Day" else 0.5
        ...
        care_day = AllocatedCareDay(
            ...
            date=care_date,
            type=day_type,
            amount_dollars=get_care_day_cost(day_type),
    ...
    def to_dict(self):
        ...
        return {
            ...
            'type': self.type,
            'amount_dollars': str(self.amount_dollars), # Convert Decimal to string for JSON
            'day_count': float(self.day_count), # Convert Decimal to float
