...
def seed_db(app):
    with app.app_context():
        ...
        allocation1 = MonthAllocation(date=date(2024, 1, 1), allocation_cents=1000000, google_sheets_child_id=1)
        allocation2 = MonthAllocation(date=date(2024, 2, 1), allocation_cents=50000, google_sheets_child_id=2)
...
def test_get_allocated_care_days_success_no_filters(client, seed_db):
    ...
    assert (
        response.json[str(allocation1.google_sheets_child_id)][0]["id"] == care_day1_1.id
        or response.json[str(allocation1.google_sheets_child_id)][0]["id"] == care_day1_2.id
    )
