...
def test_get_allocated_care_days_success_no_filters(client, seed_db):
    ...
    assert len(response.json[str(allocation1.google_sheets_child_id)]) == 2 # Two care days for child 1
    assert len(response.json[str(allocation2.google_sheets_child_id)]) == 1 # One care day for child 2
    assert response.json[str(allocation1.google_sheets_child_id)][0]['id'] == care_day1_1.id or response.json[str(allocation1.google_sheets_child_id)][0]['id'] == care_day1_2.id
    assert response.json[str(allocation2.google_sheets_child_id)][0]['id'] == care_day2_1.id
...
def test_get_allocated_care_days_filter_by_start_date(client, seed_db):
    ...
    assert len(response.json[str(allocation1.google_sheets_child_id)]) == 1 # Only care_day1_2 (2024-01-20) remains for child 1
    assert response.json[str(allocation1.google_sheets_child_id)][0]['id'] == care_day1_2.id
    assert len(response.json[str(allocation2.google_sheets_child_id)]) == 1 # care_day2_1 (2024-02-10) remains for child 2
...
def test_get_allocated_care_days_filter_by_end_date(client, seed_db):
    ...
    assert len(response.json[str(allocation1.google_sheets_child_id)]) == 1 # Only care_day1_1 (2024-01-15) remains for child 1
    assert response.json[str(allocation1.google_sheets_child_id)][0]['id'] == care_day1_1.id
...
def test_get_allocated_care_days_filter_by_start_and_end_date(client, seed_db):
    ...
    assert str(allocation1.google_sheets_child_id) in response.json
    assert str(allocation1.google_sheets_child_id + 1) not in response.json
    assert len(response.json[str(allocation1.google_sheets_child_id)]) == 1 # Only care_day1_2 (2024-01-20) remains
    assert response.json[str(allocation1.google_sheets_child_id)][0]['id'] == care_day1_2.id
