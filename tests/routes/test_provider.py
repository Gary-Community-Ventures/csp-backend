...
def seed_db(app):
    with app.app_context():
        ...
        allocation1 = MonthAllocation(date=date(2024, 1, 1), allocation_cents=1000000, child_supabase_id=1)
        allocation2 = MonthAllocation(date=date(2024, 2, 1), allocation_cents=50000, child_supabase_id=2)
...
def test_get_allocated_care_days_success_no_filters(client, seed_db):
    ...
    response = client.get(f"/provider/{care_day1_1.provider_supabase_id}/allocated_care_days")
    assert response.status_code == 200
    assert str(allocation1.child_supabase_id) in response.json  # Grouped by child_id 1
    assert str(allocation2.child_supabase_id) in response.json  # Grouped by child_id 2
    assert len(response.json[str(allocation1.child_supabase_id)]) == 2  # Two care days for child 1
    assert len(response.json[str(allocation2.child_supabase_id)]) == 1  # One care day for child 2
    assert (
        response.json[str(allocation1.child_supabase_id)][0]["id"] == care_day1_1.id
        or response.json[str(allocation1.child_supabase_id)][0]["id"] == care_day1_2.id
    )
    assert response.json[str(allocation2.child_supabase_id)][0]["id"] == care_day2_1.id


def test_get_allocated_care_days_filter_by_child_id(client, seed_db):
    allocation1, _, care_day1_1, _, _ = seed_db
    response = client.get(
        f"/provider/{care_day1_1.provider_supabase_id}/allocated_care_days?childId={allocation1.child_supabase_id}"
    )
    assert response.status_code == 200
    assert str(allocation1.child_supabase_id) in response.json
    assert str(allocation1.child_supabase_id) + "x" not in response.json
    assert len(response.json[str(allocation1.child_supabase_id)]) == 2
...
def test_get_allocated_care_days_filter_by_start_date(client, seed_db):
    ...
    response = client.get(f"/provider/{care_day1_1.provider_supabase_id}/allocated_care_days?startDate=2024-01-16")
    assert response.status_code == 200
    assert str(allocation1.child_supabase_id) in response.json
    assert str(allocation2.child_supabase_id) in response.json
    assert (
...
def test_get_allocated_care_days_filter_by_end_date(client, seed_db):
    ...
    response = client.get(f"/provider/{care_day1_1.provider_supabase_id}/allocated_care_days?endDate=2024-01-18")
    assert response.status_code == 200
    assert str(allocation1.child_supabase_id) in response.json
    assert str(allocation2.child_supabase_id) not in response.json  # Child 2 care day is after end date
    assert (
        len(response.json[str(allocation1.child_supabase_id)]) == 1
    )  # Only care_day1_1 (2024-01-15) remains for child 1
    assert response.json[str(allocation1.child_supabase_id)][0]["id"] == care_day1_1.id


def test_get_allocated_care_days_filter_by_start_and_end_date(client, seed_db):
    allocation1, _, _, care_day1_2, _ = seed_db
    response = client.get(
        f"/provider/{care_day1_2.provider_supabase_id}/allocated_care_days?startDate=2024-01-16&endDate=2024-01-22"
    )
    assert response.status_code == 200
    assert str(allocation1.child_supabase_id) in response.json
    assert str(allocation1.child_supabase_id) + "X" not in response.json
    assert len(response.json[str(allocation1.child_supabase_id)]) == 1  # Only care_day1_2 (2024-01-20) remains
    assert response.json[str(allocation1.child_supabase_id)][0]["id"] == care_day1_2.id
...
def test_get_allocated_care_days_filter_all_params(client, seed_db):
    allocation1, _, _, care_day1_2, _ = seed_db
    response = client.get(
        f"/provider/{care_day1_2.provider_supabase_id}/allocated_care_days?childId={allocation1.child_supabase_id}&startDate=2024-01-16&endDate=2024-01-22"
    )
    assert response.status_code == 200
    assert str(allocation1.child_supabase_id) in response.json
    assert str(allocation1.child_supabase_id) + "X" not in response.json
...
def test_get_allocated_care_days_no_results(client, seed_db):
    ...
    response = client.get(
        f"/provider/{care_day1_1.provider_supabase_id}/allocated_care_days?startDate=2025-01-01"
    )  # Future date
    assert response.status_code == 200
...
def test_get_allocated_care_days_invalid_end_date(client, seed_db):
    _, _, care_day1_1, _, _ = seed_db
    response = client.get(f"/provider/{care_day1_1.provider_supabase_id}/allocated_care_days?endDate=invalid-date")
    assert response.status_code == 400
